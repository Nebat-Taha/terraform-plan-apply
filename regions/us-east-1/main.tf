...
# NEW: Call the Security Group Module for SSH access
...
# NEW: Call the EC2 instance module
module "web_server_instance" {
  source             = "../../modules/ec2"
  ami_id             = var.ec2_ami_id
  instance_type      = var.ec2_instance_type
  key_name           = module.regional_key_pair.key_pair_name
  subnet_id          = module.vpc_network.app_subnet_id              # NEW: Use app subnet
  security_group_ids = [module.ssh_security_group.security_group_id] # NEW: Attach SSH SG
  name_tag           = "${var.environment}-app-server"               # Renamed to reflect app tier
  environment_tag    = var.environment
  region_tag         = var.aws_region
}

# Update Outputs to reflect new network resources
output "vpc_id" {
...
# Update outputs to reference the new module's outputs
